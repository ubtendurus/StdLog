@extends('layouts.clock')
        ...
        <div class="fixedcenter">
            <div class="clockwrapper">
                <div class="clockinout">
                    <button class="btnclock timein active" data-type="timein">{{ __("Check In") }}</button>
                    <button class="btnclock timeout" data-type="timeout">{{ __("Check Out") }}</button>
                </div>
            </div>
            <!--<div class="clockwrapper">
                <div class="timeclock">
                ...
                <table width="100%" class="table table-striped table-hover" id="dataTables-example" data-order='[[ 0, "desc" ]]'>
                    ...
                    <tbody>
                        ...
                        @foreach ($data1 as $student)
                            ...
                            <!--<a href="{{ url('/profile/view/'.$student->reference) }}" class=utend "ui circular basic icon button tiny"><i class="file alternate outline icon"></i></a>-->
                ...
                <div class="userinput">
                    <form action="" method="get" accept-charset="utf-8" class="ui form">
                        ...
                        <div class="inline field">
                            <!--<input @if($rfid == 'on') id="rfid" @endif class="enter_idno uppercase @if($rfid == 'on') mr-0 @endif" name="idno" value="" type="text" placeholder="{{ __("ENTER YOUR SCHOOL ID") }}" required autofocus>-->
    ...
    $('.btnclock').click(function(event) {
        ...
        if (is_comment == "timein" || is_comment == "timeout") {
        ...
        $.ajax({ url: url + '/attendance/add', type: 'post', dataType: 'json', data: {idno: idno, type: type, clockin_comment: comment}, headers: { 'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content') },
            ...
            success: function(response) {
                if(response['error'] != null) 
                {
                    ...
                    $('#fullname').text(response['student']);
                    $('.message-after').slideToggle().slideDown('400');
                } else {
                    function type(clocktype) {
                        if (clocktype == "timein") {
                            return "{{ __('Check In at') }}";
                        } else {
                            return "{{ __('Check Out at') }}";
                        }
                    }
