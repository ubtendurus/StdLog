@extends('layouts.default')
    ...
    @section('meta')
        <title>Log | Log Tracker</title>
        <meta name="description" content="Log Tracker log, view all student attendances, clock-in, edit, and delete attendances.">
    @endsection
            ...
            success: function(response) {
                ...
                function showdata(jsonresponse) {
                    ...
                    for (var i = 0; i < student.length; i++) {
                        var time_in = student[i].timein;
                        var time_out = student[i].timeout;
                        var in_status = student[i].status_timein;
                        var out_status = student[i].status_timeout;
                        ...
                        function d_status(in_status, out_status) {
                            if(in_status != '' && out_status != '') {
                                return "<span class=' " + t_in_status(in_status) + "'>" +student[i].status_timein+ "</span>" + ' / ' + "<span class='" + t_out_status(out_status) + "'>" +student[i].status_timeout+ "</span>";
                            } else if (in_status != '' && out_status == '') {
                                return "<span class=' " + t_in_status(in_status) + "'>" +student[i].status_timein+ "</span>";
                            } else {
                                return "";
                            }
                        }
                        ...
                        if (cc === "on") {
                            tbody.append("<tr>"+ 
                                    ...
                                    "<td>"+student[i].student+"</td>" + 
                                    "<td>"+time(1)+"</td>" + 
                                    "<td>"+time(2)+"</td>" + 
                                    "<td>"+th(student[i].totalhours)+"</td>" + 
                                    "<td>"+d_status(in_status, out_status)+"</td>" + 
                                    "<td>"+student[i].comment+"</td>" + 
                                    "<td class='align-right'><a href='"+ url + "/attendance/edit/" + student[i].id +"' class='ui circular basic icon button tiny'><i class='edit outline icon'></i></a> <a href='"+ url + "/attendance/delete/" + student[i].id +"' class='ui circular basic icon button tiny'><i class='trash alternate outline icon'></i></a>" +
                                    "</td>"+
                                "</tr>");
                        } else {
                            tbody.append("<tr>"+ 
